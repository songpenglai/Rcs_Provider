...
public class MmsSmsDatabaseHelper extends SQLiteOpenHelper {
	...
	private static final String UPDATE_THREAD_COUNT_ON_NEW = "  UPDATE threads SET message_count = "
	...
	private static final String UPDATE_THREAD_COUNT_ON_OLD = "  UPDATE threads SET message_count = "
			+ "     (SELECT COUNT(sms._id) FROM sms LEFT JOIN threads "
			+ "      ON threads._id = "
			+ Sms.THREAD_ID
			+ "      WHERE "
	...
	private static final String SMS_UPDATE_THREAD_DATE_SNIPPET_COUNT_ON_UPDATE = "BEGIN"
			...
			+ Sms.THREAD_ID
			+ "; "
			+ UPDATE_THREAD_COUNT_ON_NEW + SMS_UPDATE_THREAD_READ_BODY + "END;";

	private static final String PDU_UPDATE_THREAD_CONSTRAINTS = "  WHEN new."
			+ Mms.MESSAGE_TYPE + "=" + PduHeaders.MESSAGE_TYPE_RETRIEVE_CONF
			+ "    OR new." + Mms.MESSAGE_TYPE + "="
			+ PduHeaders.MESSAGE_TYPE_NOTIFICATION_IND + "    OR new."
			+ Mms.MESSAGE_TYPE + "=" + PduHeaders.MESSAGE_TYPE_SEND_REQ + " ";

	// When looking in the pdu table for unread messages, only count messages
	// that
	// are displayed to the user. The constants are defined in PduHeaders and
	// could be used
	// here, but the string "(m_type=132 OR m_type=130 OR m_type=128)" is used
	// throughout this
	...
	private static final String UPDATE_THREAD_SNIPPET_SNIPPET_CS_ON_DELETE = "  UPDATE threads SET snippet = "
			+ "   (SELECT snippet FROM"
	...
	private static final String PART_UPDATE_THREADS_ON_INSERT_TRIGGER = "CREATE TRIGGER update_threads_on_insert_part "
	...
	// When the 'mid' column in the part table is updated, we need to run the
	// trigger to update
	// the threads table's has_attachment column, if the part is an attachment.
	private static final String PART_UPDATE_THREADS_ON_UPDATE_TRIGGER = "CREATE TRIGGER update_threads_on_update_part "
			+ " AFTER UPDATE of "
			+ Part.MSG_ID
			+ " ON part "
			+ " WHEN new.ct != 'text/plain' AND new.ct != 'application/smil' "
	...
	private static final String PART_UPDATE_THREADS_ON_DELETE_TRIGGER = "CREATE TRIGGER update_threads_on_delete_part "
	...
	// When the 'thread_id' column in the pdu table is updated, we need to run
	// the trigger to update
	// the threads table's has_attachment column, if the message has an
	// attachment in 'part' table
	private static final String PDU_UPDATE_THREADS_ON_UPDATE_TRIGGER = "CREATE TRIGGER update_threads_on_update_pdu "
	 ...
	 */
	/* package */static synchronized MmsSmsDatabaseHelper getInstance(
			Context context) {
		if (sInstance == null) {
	...
	/**
	 * Look through all the recipientIds referenced by the threads and then
	 * delete any unreferenced rows from the canonical_addresses table.
	 */
	private static void removeUnferencedCanonicalAddresses(SQLiteDatabase db) {
		Cursor c = db.query(MmsSmsProvider.TABLE_THREADS,
				new String[] { "recipient_ids" }, null, null, null, null, null);
		if (c != null) {
			try {
				if (c.getCount() == 0) {
					...
					// Find all the referenced recipient_ids from the threads.
					// recipientIds is
					...
					// Now build a selection string of all the unique recipient
					// ids
					...
					if (sb.length() > 0) {
						int rows = db.delete("canonical_addresses",
								sb.toString(), null);
					}
				}
			} finally {
		...
		try {
			...
			int rows = db.delete(MmsSmsProvider.TABLE_THREADS,
					"_id = ? AND _id NOT IN"
							...
							+ "           UNION SELECT thread_id FROM groups "
							+ "           UNION SELECT thread_id FROM pdu)",
					new String[] { String.valueOf(thread_id) });
			if (rows > 0) {
				// If this deleted a row, let's remove orphaned
				// canonical_addresses and get outta here
				...
				db.execSQL("  UPDATE threads SET message_count = "
						+ "     (SELECT COUNT(sms._id) FROM sms LEFT JOIN threads "
						+ "      ON threads._id = "
						+ Sms.THREAD_ID
						+ "      WHERE "
				...
				// First check to see if there are any messages with errors in
				// this thread.
				...
				if (c != null) {
					try {
						setError = c.getCount(); // Because of the LIMIT 1,
													// count will be 1 or 0.
					} finally {
						c.close();
					}
				}
				// What's the current state of the error flag in the threads
				// table?
				String errorQuery = "SELECT error FROM threads WHERE _id = "
						+ thread_id;
					...
					try {
						if (c.moveToNext()) {
							...
							if (curError != setError) {
								// The current thread error column differs,
								// update it.
								db.execSQL("UPDATE threads SET error="
										+ setError + " WHERE _id = "
										+ thread_id);
							}
						}
	...
	public static void updateAllThreads(SQLiteDatabase db, String where,
		...
		try {
			...
			String query = "SELECT _id FROM threads WHERE _id IN "
					+ "(SELECT DISTINCT thread_id FROM rcs " + where + ")";
			...
			// TODO: there are several db operations in this function. Lets wrap
			// them in a
			// transaction to make it faster.
			// remove orphaned threads
			db.delete(
					MmsSmsProvider.TABLE_THREADS,
	...
	public static int deleteOneSms(SQLiteDatabase db, int message_id) {
		...
		Cursor c = db.query("sms", new String[] { "thread_id" }, "_id="
				+ message_id, null, null, null, null);
		if (c != null) {
			if (c.moveToFirst()) {
	...
	private void populateWordsTable(SQLiteDatabase db) {
		...
		{
			Cursor smsRows = db.query("sms",
					new String[] { Sms._ID, Sms.BODY }, null, null, null, null,
					null);
			try {
				if (smsRows != null) {
					...
					while (smsRows.moveToNext()) {
						...
						db.insert(TABLE_WORDS,
								Telephony.MmsSms.WordsTable.INDEXED_TEXT, cv);
					}
				}
			} finally {
				if (smsRows != null) {
		...
		{
			Cursor mmsRows = db.query("part", new String[] { Part._ID,
					Part.TEXT }, "ct = 'text/plain'", null, null, null, null);
			try {
				if (mmsRows != null) {
	...
	private void createWordsTables(SQLiteDatabase db) {
		try {
			...
			// NOTE don't handle inserts using a trigger because it has an
			// unwanted
			// side effect: the value returned for the last row ends up being
			// the
	...
	private void createMmsTables(SQLiteDatabase db) {
		...
		db.execSQL("CREATE TABLE " + MmsProvider.TABLE_PDU + " (" + Mms._ID
				+ " INTEGER PRIMARY KEY AUTOINCREMENT," + Mms.THREAD_ID
				+ " INTEGER," + Mms.DATE + " INTEGER," + Mms.DATE_SENT
				+ " INTEGER DEFAULT 0," + Mms.MESSAGE_BOX + " INTEGER,"
				+ Mms.READ + " INTEGER DEFAULT 0," + Mms.MESSAGE_ID + " TEXT,"
				+ Mms.SUBJECT + " TEXT," + Mms.SUBJECT_CHARSET + " INTEGER,"
				+ Mms.CONTENT_TYPE + " TEXT," + Mms.CONTENT_LOCATION + " TEXT,"
				+ Mms.EXPIRY + " INTEGER," + Mms.MESSAGE_CLASS + " TEXT,"
				+ Mms.MESSAGE_TYPE + " INTEGER," + Mms.MMS_VERSION
				+ " INTEGER," + Mms.MESSAGE_SIZE + " INTEGER," + Mms.PRIORITY
				+ " INTEGER," + Mms.READ_REPORT + " INTEGER,"
				+ Mms.REPORT_ALLOWED + " INTEGER," + Mms.RESPONSE_STATUS
				+ " INTEGER," + Mms.STATUS + " INTEGER," + Mms.TRANSACTION_ID
				+ " TEXT," + Mms.RETRIEVE_STATUS + " INTEGER,"
				+ Mms.RETRIEVE_TEXT + " TEXT," + Mms.RETRIEVE_TEXT_CHARSET
				+ " INTEGER," + Mms.READ_STATUS + " INTEGER,"
				+ Mms.CONTENT_CLASS + " INTEGER," + Mms.RESPONSE_TEXT
				+ " TEXT," + Mms.DELIVERY_TIME + " INTEGER,"
				+ Mms.DELIVERY_REPORT + " INTEGER," + Mms.LOCKED
				+ " INTEGER DEFAULT 0," + Mms.SEEN + " INTEGER DEFAULT 0,"
				+ Mms.TEXT_ONLY + " INTEGER DEFAULT 0," + Mms.SUB_ID
				+ " INTEGER DEFAULT 0" + ");");

		db.execSQL("CREATE TABLE " + MmsProvider.TABLE_ADDR + " (" + Addr._ID
				+ " INTEGER PRIMARY KEY," + Addr.MSG_ID + " INTEGER,"
				+ Addr.CONTACT_ID + " INTEGER," + Addr.ADDRESS + " TEXT,"
				+ Addr.TYPE + " INTEGER," + Addr.CHARSET + " INTEGER);");

		db.execSQL("CREATE TABLE " + MmsProvider.TABLE_PART + " (" + Part._ID
				+ " INTEGER PRIMARY KEY AUTOINCREMENT," + Part.MSG_ID
				+ " INTEGER," + Part.SEQ + " INTEGER DEFAULT 0,"
				+ Part.CONTENT_TYPE + " TEXT," + Part.NAME + " TEXT,"
				+ Part.CHARSET + " INTEGER," + Part.CONTENT_DISPOSITION
				+ " TEXT," + Part.FILENAME + " TEXT," + Part.CONTENT_ID
				+ " TEXT," + Part.CONTENT_LOCATION + " TEXT," + Part.CT_START
				+ " INTEGER," + Part.CT_TYPE + " TEXT," + Part._DATA + " TEXT,"
				+ Part.TEXT + " TEXT);");
	...
	// Unlike the other trigger-creating functions, this function can be called
	// multiple times
	// without harm.
	private void createMmsTriggers(SQLiteDatabase db) {
		...
		db.execSQL("CREATE TRIGGER part_cleanup DELETE ON "
				+ MmsProvider.TABLE_PDU + " " + "BEGIN " + "  DELETE FROM "
				+ MmsProvider.TABLE_PART + "  WHERE " + Part.MSG_ID
				+ "=old._id;" + "END;");

		// Cleans up address info when a MM is deleted.
		db.execSQL("DROP TRIGGER IF EXISTS addr_cleanup");
		db.execSQL("CREATE TRIGGER addr_cleanup DELETE ON "
				+ MmsProvider.TABLE_PDU + " " + "BEGIN " + "  DELETE FROM "
				+ MmsProvider.TABLE_ADDR + "  WHERE " + Addr.MSG_ID
				+ "=old._id;" + "END;");
		...
		db.execSQL("CREATE TRIGGER cleanup_delivery_and_read_report "
				...
				+ ")"
				+ "    AND "
				+ Mms.MESSAGE_ID + "=old." + Mms.MESSAGE_ID + "; " + "END;");
		...
		db.execSQL("CREATE TRIGGER delete_mms_pending_on_delete "
				+ "AFTER DELETE ON " + MmsProvider.TABLE_PDU + " " + "BEGIN "
				+ "  DELETE FROM " + MmsSmsProvider.TABLE_PENDING_MSG
				+ "  WHERE " + PendingMessages.MSG_ID + "=old._id; " + "END;");
		...
		db.execSQL("CREATE TRIGGER delete_mms_pending_on_update "
				...
				+ "  AND new." + Mms.MESSAGE_BOX + "!="
				+ Mms.MESSAGE_BOX_OUTBOX + " " + "BEGIN " + "  DELETE FROM "
				+ MmsSmsProvider.TABLE_PENDING_MSG + "  WHERE "
				+ PendingMessages.MSG_ID + "=new._id; " + "END;");
		...
		db.execSQL("CREATE TRIGGER pdu_update_thread_on_delete "
				+ "AFTER DELETE ON pdu " + "BEGIN " + "  UPDATE threads SET "
				+ "     date = (strftime('%s','now') * 1000)"
		...
		db.execSQL("CREATE TRIGGER pdu_update_thread_read_on_update AFTER"
				+ "  UPDATE OF " + Mms.READ + "  ON " + MmsProvider.TABLE_PDU
				+ " " + PDU_UPDATE_THREAD_CONSTRAINTS + "BEGIN "
				+ PDU_UPDATE_THREAD_READ_BODY + "END;");
	...
	private void createSmsTables(SQLiteDatabase db) {
		...
		db.execSQL("CREATE TABLE sms ("
				...
				+ "recv_size INTEGER DEFAULT 0,"
				+ "imdn_string TEXT,"
				+ "imdn_type INTEGER DEFAULT 0,"
				+ "notify_type INTEGER DEFAULT 0,"
				+ "group_chat_id TEXT,"
				+ "has_thums INTEGER DEFAULT 0"
				+ ");");

		/**
		 * This table is used by the SMS dispatcher to hold incomplete partial
		 * messages until all the parts arrive.
		 */
		db.execSQL("CREATE TABLE raw (" + "_id INTEGER PRIMARY KEY,"
				+ "date INTEGER," + "reference_number INTEGER," + // one per
																	// full
																	// message
				"count INTEGER," + // the number of parts
				"sequence INTEGER," + // the part number of this message
				"destination_port INTEGER," + "address TEXT," + "pdu TEXT);"); // the
																				// raw
																				// PDU
																				// for
																				// this
		...
		db.execSQL("CREATE TABLE attachments (" + "sms_id INTEGER,"
				+ "content_url TEXT," + "offset INTEGER);");

		/**
		 * This table is used by the SMS dispatcher to hold pending delivery
		 * status report intents.
		 */
		db.execSQL("CREATE TABLE sr_pending (" + "reference_number INTEGER,"
...
//				+ "imdn_type INTEGER DEFAULT 0,"
//				+ "notify_type INTEGER DEFAULT 0,"
//				+ "abbreviative INTEGER DEFAULT 0"
//				+ ");");
...
//				+ ");");
		...
		db.execSQL("CREATE TABLE groups ("
				...
				+ "name TEXT,"
				+ "type INTEGER,"
				+ "organicer_phone TEXT,"
				+ "begin_time INTEGER ,"
				+ "end_time INTEGER ,"
				+ "duration TEXT," 
				+ "thread_id INTEGER ,"
				+ "group_id TEXT ," 
				+ "session_idertity TEXT,"
				+ "group_chat_id TEXT,"
				+ "group_member TEXT"
				+ ");");
	}
	private void createCommonTables(SQLiteDatabase db) {
		...
		/**
		 * This table maps the first instance seen of any particular MMS/SMS
		 * address to an ID, which is then used as its canonical representation.
		 * If the same address or an equivalent address (as determined by our
		 * Sqlite PHONE_NUMBERS_EQUAL extension) is seen later, this same ID
		 * will be used. The _id is created with AUTOINCREMENT so it will never
		 * be reused again if a recipient is deleted.
		 */
		db.execSQL("CREATE TABLE canonical_addresses ("
				+ "_id INTEGER PRIMARY KEY AUTOINCREMENT," + "address TEXT);");

		/**
		 * This table maps the subject and an ordered set of recipient IDs,
		 * separated by spaces, to a unique thread ID. The IDs come from the
		 * canonical_addresses table. This works because messages are considered
		 * to be part of the same thread if they have the same subject (or a
		 * null subject) and the same set of recipients.
		 */
		db.execSQL("CREATE TABLE threads (" + Threads._ID
				+ " INTEGER PRIMARY KEY AUTOINCREMENT," + Threads.DATE
				+ " INTEGER DEFAULT 0," + Threads.MESSAGE_COUNT
				+ " INTEGER DEFAULT 0," + Threads.RECIPIENT_IDS + " TEXT,"
				+ Threads.SNIPPET + " TEXT," + Threads.SNIPPET_CHARSET
				+ " INTEGER DEFAULT 0," + Threads.READ + " INTEGER DEFAULT 1,"
				+ Threads.TYPE + " INTEGER DEFAULT 0," + Threads.ERROR
				+ " INTEGER DEFAULT 0," + Threads.HAS_ATTACHMENT
				+ " INTEGER DEFAULT 0);");
	...
	private void createCommonTriggers(SQLiteDatabase db) {
		...
		db.execSQL("CREATE TRIGGER sms_update_thread_date_subject_on_update AFTER"
				...
				+ ", "
				+ Sms.TYPE
				+ "  ON sms " + SMS_UPDATE_THREAD_DATE_SNIPPET_COUNT_ON_UPDATE);

		// Updates threads table whenever a message in sms is updated.
		db.execSQL("CREATE TRIGGER sms_update_thread_read_on_update AFTER"
				+ "  UPDATE OF " + Sms.READ + "  ON sms " + "BEGIN "
				+ SMS_UPDATE_THREAD_READ_BODY + "END;");

		// As of DATABASE_VERSION 55, we've removed these triggers that delete
		// empty threads.
		// These triggers interfere with saving drafts on brand new threads.
		// Instead of
		// triggers cleaning up empty threads, the empty threads should be
		// cleaned up by
		...
		db.execSQL("CREATE TRIGGER update_threads_error_on_update_mms "
				...
				+ "BEGIN" + "  UPDATE threads SET error = " + "    CASE"
				+ "      WHEN NEW.err_type >= 10 THEN error + 1"
				+ "      ELSE error - 1" + "    END " + "  WHERE _id ="
				+ "   (SELECT DISTINCT thread_id" + "    FROM pdu"
	...
	private void upgradeDatabaseToVersion45(SQLiteDatabase db) {
		...
		db.execSQL("ALTER TABLE sms ADD COLUMN " + Sms.LOCKED
				+ " INTEGER DEFAULT 0");

		// Add 'locked' column to pdu table.
		db.execSQL("ALTER TABLE pdu ADD COLUMN " + Mms.LOCKED
				+ " INTEGER DEFAULT 0");
	}

	private void upgradeDatabaseToVersion46(SQLiteDatabase db) {
		// add the "text" column for caching inline text (e.g. strings) instead
		// of
		// putting them in an external file
		db.execSQL("ALTER TABLE part ADD COLUMN " + Part.TEXT + " TEXT");

		Cursor textRows = db.query("part", new String[] { Part._ID, Part._DATA,
				Part.TEXT }, "ct = 'text/plain' OR ct == 'application/smil'",
				null, null, null, null);
		...
		try {
			...
			if (textRows != null) {
				...
				// This code is imperfect in that we can't guarantee that all
				// the
				// backing files get deleted. For example if the system aborts
				// after
				...
				while (textRows.moveToNext()) {
					...
					if (path != null) {
						try {
							...
							db.execSQL("UPDATE part SET " + Part._DATA
									+ " = NULL, " + Part.TEXT + " = ?",
									new String[] { v.getString() });
							is.close();
			...
			for (String pathToDelete : filesToDelete) {
				try {
					...
					Log.e(TAG, "unable to clean up old mms file for "
							+ pathToDelete, ex);
				}
			}
			if (textRows != null) {
	...
	private void upgradeDatabaseToVersion51(SQLiteDatabase db) {
		...
		try {
			// update the existing sms and pdu tables so the new "seen" column
			// is the same as
			// the "read" column for each row.
			ContentValues contentValues = new ContentValues();
			contentValues.put("seen", 1);
			int count = db.update("sms", contentValues, "read=1", null);
			Log.d(TAG, "[MmsSmsDb] upgradeDatabaseToVersion51: updated "
					+ count + " rows in sms table to have READ=1");
			count = db.update("pdu", contentValues, "read=1", null);
			Log.d(TAG, "[MmsSmsDb] upgradeDatabaseToVersion51: updated "
					+ count + " rows in pdu table to have READ=1");
		} catch (Exception ex) {
	...
	private void upgradeDatabaseToVersion54(SQLiteDatabase db) {
		...
		db.execSQL("ALTER TABLE sms ADD COLUMN " + Sms.DATE_SENT
				+ " INTEGER DEFAULT 0");

		// Add 'date_sent' column to pdu table.
		db.execSQL("ALTER TABLE pdu ADD COLUMN " + Mms.DATE_SENT
				+ " INTEGER DEFAULT 0");
	}
	...
	private void upgradeDatabaseToVersion57(SQLiteDatabase db) {
		...
		db.execSQL("DELETE FROM " + MmsProvider.TABLE_PDU + " WHERE "
				+ Mms.THREAD_ID + " IS NULL");
	}
	...
	public synchronized SQLiteDatabase getWritableDatabase() {
		...
		if (!sTriedAutoIncrement) {
			...
			boolean hasAutoIncrementThreads = hasAutoIncrement(db,
					MmsSmsProvider.TABLE_THREADS);
			boolean hasAutoIncrementAddresses = hasAutoIncrement(db,
					"canonical_addresses");
			...
			Log.d(TAG, "[getWritableDatabase] hasAutoIncrementThreads: "
					+ hasAutoIncrementThreads + " hasAutoIncrementAddresses: "
					+ hasAutoIncrementAddresses + " hasAutoIncrementPart: "
					+ hasAutoIncrementPart + " hasAutoIncrementPdu: "
					+ hasAutoIncrementPdu);
			...
			if (!hasAutoIncrementThreads) {
				...
				try {
					if (false && sFakeLowStorageTest) {
						Log.d(TAG,
								"[getWritableDatabase] mFakeLowStorageTest is true "
										+ " - fake exception");
						throw new Exception("FakeLowStorageTest");
					}
					upgradeThreadsTableToAutoIncrement(db); // a no-op if
															// already upgraded
					db.setTransactionSuccessful();
				} catch (Throwable ex) {
					Log.e(TAG,
			...
			if (!hasAutoIncrementAddresses) {
				...
				try {
					...
					upgradeAddressTableToAutoIncrement(db); // a no-op if
															// already upgraded
					db.setTransactionSuccessful();
				} catch (Throwable ex) {
					Log.e(TAG,
			...
			if (!hasAutoIncrementPart) {
				...
				try {
					...
					upgradePartTableToAutoIncrement(db); // a no-op if already
															// upgraded
					db.setTransactionSuccessful();
				} catch (Throwable ex) {
					Log.e(TAG,
							"Failed to add autoIncrement to part: "
									+ ex.getMessage(), ex);
			...
			if (!hasAutoIncrementPdu) {
				...
				try {
					...
					upgradePduTableToAutoIncrement(db); // a no-op if already
														// upgraded
					db.setTransactionSuccessful();
				} catch (Throwable ex) {
					Log.e(TAG,
							"Failed to add autoIncrement to pdu: "
									+ ex.getMessage(), ex);
			...
			try {
				if (c.moveToFirst()) {
					...
					result = schema != null ? schema.contains("AUTOINCREMENT")
							: false;
					Log.d(TAG, "[MmsSmsDb] tableName: " + tableName
	...
	// upgradeThreadsTableToAutoIncrement() is called to add the AUTOINCREMENT
	// keyword to
	// the threads table. This could fail if the user has a lot of conversations
	// and not enough
	// storage to make a copy of the threads table. That's ok. This upgrade is
	// optional. It'll
	...
	private void upgradeThreadsTableToAutoIncrement(SQLiteDatabase db) {
		if (hasAutoIncrement(db, MmsSmsProvider.TABLE_THREADS)) {
			Log.d(TAG,
					"[MmsSmsDb] upgradeThreadsTableToAutoIncrement: already upgraded");
			return;
		}
		Log.d(TAG, "[MmsSmsDb] upgradeThreadsTableToAutoIncrement: upgrading");

		// Make the _id of the threads table autoincrement so we never re-use
		// thread ids
		// Have to create a new temp threads table. Copy all the info from the
		// old table.
		// Drop the old table and rename the new table to that of the old.
		db.execSQL("CREATE TABLE threads_temp (" + Threads._ID
				+ " INTEGER PRIMARY KEY AUTOINCREMENT," + Threads.DATE
				+ " INTEGER DEFAULT 0," + Threads.MESSAGE_COUNT
				+ " INTEGER DEFAULT 0," + Threads.RECIPIENT_IDS + " TEXT,"
				+ Threads.SNIPPET + " TEXT," + Threads.SNIPPET_CHARSET
	...
	// upgradeAddressTableToAutoIncrement() is called to add the AUTOINCREMENT
	// keyword to
	// the canonical_addresses table. This could fail if the user has a lot of
	// people they've
	// messaged with and not enough storage to make a copy of the
	// canonical_addresses table.
	// That's ok. This upgrade is optional. It'll be called again next time the
	// device is rebooted.
	private void upgradeAddressTableToAutoIncrement(SQLiteDatabase db) {
		if (hasAutoIncrement(db, "canonical_addresses")) {
			Log.d(TAG,
					"[MmsSmsDb] upgradeAddressTableToAutoIncrement: already upgraded");
			return;
		}
		Log.d(TAG, "[MmsSmsDb] upgradeAddressTableToAutoIncrement: upgrading");

		// Make the _id of the canonical_addresses table autoincrement so we
		// never re-use ids
		// Have to create a new temp canonical_addresses table. Copy all the
		// info from the old
		// table. Drop the old table and rename the new table to that of the
		// old.
	...
	// upgradePartTableToAutoIncrement() is called to add the AUTOINCREMENT
	// keyword to
	// the part table. This could fail if the user has a lot of
	// sound/video/picture attachments
	...
	private void upgradePartTableToAutoIncrement(SQLiteDatabase db) {
		if (hasAutoIncrement(db, "part")) {
			Log.d(TAG,
					"[MmsSmsDb] upgradePartTableToAutoIncrement: already upgraded");
			return;
		}
		...
		// table. Drop the old table and rename the new table to that of the
		// old.
		db.execSQL("CREATE TABLE part_temp (" + Part._ID
				+ " INTEGER PRIMARY KEY AUTOINCREMENT," + Part.MSG_ID
				+ " INTEGER," + Part.SEQ + " INTEGER DEFAULT 0,"
				+ Part.CONTENT_TYPE + " TEXT," + Part.NAME + " TEXT,"
				+ Part.CHARSET + " INTEGER," + Part.CONTENT_DISPOSITION
		...
		// part-related triggers get tossed when the part table is dropped --
		// rebuild them.
		createMmsTriggers(db);
	}

	// upgradePduTableToAutoIncrement() is called to add the AUTOINCREMENT
	// keyword to
	...
	private void upgradePduTableToAutoIncrement(SQLiteDatabase db) {
		if (hasAutoIncrement(db, "pdu")) {
			Log.d(TAG,
					"[MmsSmsDb] upgradePduTableToAutoIncrement: already upgraded");
			return;
		}
		...
		// table. Drop the old table and rename the new table to that of the
		// old.
		db.execSQL("CREATE TABLE pdu_temp (" + Mms._ID
				+ " INTEGER PRIMARY KEY AUTOINCREMENT," + Mms.THREAD_ID
				+ " INTEGER," + Mms.DATE + " INTEGER," + Mms.DATE_SENT
				+ " INTEGER DEFAULT 0," + Mms.MESSAGE_BOX + " INTEGER,"
				+ Mms.READ + " INTEGER DEFAULT 0," + Mms.MESSAGE_ID + " TEXT,"
		...
		// pdu-related triggers get tossed when the part table is dropped --
		// rebuild them.
		createMmsTriggers(db);
	}
